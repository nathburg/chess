  ...
  for (let rookPosition in castling[currentPlayer]) {
    if (
      ...
      if (
        ...
        JSON.stringify(castling[currentPlayer][rookPosition].spacesBetween)
      ) {
  ...
  if (inRange(pawnSpecs[currentPlayer].moveDirection(y))) {
    ...
    if (
      ...
      if (
        ...
        inspectSpace(coordsToString([x, doubleMove(y)])).condition === "empty"
      ) {
...
function threatsToSpace(space) {
  ...
  for (let position in board) {
    if (board[position].color === currentPlayer) {
      ...
      const checkArray = pieceStringToFunction[board[position].piece](position);
